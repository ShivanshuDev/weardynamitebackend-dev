...
class NotificationService {
     ...
     */
    static async sendOrderConfirmed(order, items, user) {
        // 1. Generate Invoice PDF
        const pdfBuffer = await pdfService_1.PDFService.generateInvoice({ ...order, items }).catch(err => {
            console.error('[PDF ERROR] Failed to generate invoice:', err);
            return undefined;
        });
        // 2. Save to Inbox
        await (0, user_service_1.saveUserNotification)(user.id || user.user_id, {
        ...
        await mailService_1.MailService.sendOrderConfirmedEmail(user.email, user.name, order, items, pdfBuffer);
    ...
    static async sendOrderStatusUpdate(order, status, user) {
        ...
        // 2. Send Email
        await mailService_1.MailService.sendOrderStatusEmail(user.email, user.name, order, status);
        // 3. Send Push
        if (user.fcmToken) {
            await this.sendPush(user.fcmToken, {
                title: title,
     ...
     */
    static async broadcastNewProduct(product, users) {
        ...
        const emailPromises = users.slice(0, 100).map(u => mailService_1.MailService.sendNewProductEmail(u.email, u.name, product));
        const inboxPromises = users.map(u => (0, user_service_1.saveUserNotification)(u.id || u.user_id, {
            title: 'New Drop Alert! 🔥',
     ...
     */
    static async sendBirthdayWish(user) {
        // 1. Save to Inbox
        await (0, user_service_1.saveUserNotification)(user.id || user.user_id, {
        ...
        // 2. Send Email
        await mailService_1.MailService.sendBirthdayEmail(user.email, user.name);
        // 3. Send Push
        if (user.fcmToken) {
            await this.sendPush(user.fcmToken, {
                title: 'Happy Birthday! 🎂',
     ...
     */
    static async broadcastCustomNotification(payload, users) {
        ...
        for (const user of users) {
            ...
            if (userId) {
                ...
                inboxPromises.push((0, user_service_1.saveUserNotification)(userId, {
                    title: payload.title,
            ...
            if (payload.channels.includes('email') && user.email) {
                emailPromises.push(mailService_1.MailService.sendCustomBroadcastEmail(user.email, user.name, payload.title, payload.body, payload.image, payload.product).then(() => results.emailSent++));
            }
        }
     ...
     */
    static async sendAdminInquiryNotification(inquiry) {
        const adminId = 'MASTER-ADMIN';
        const isBulk = inquiry.type === 'bulk_order';
        // 1. In-App Notification
        await (0, user_service_1.saveUserNotification)(adminId, {
        ...
        if (isBulk) {
            await mailService_1.MailService.sendBulkInquiryNotification(inquiry);
        }
        else {
            await mailService_1.MailService.sendStandardInquiryAdminNotification(inquiry);
        }
        // 3. Push Notification
        try {
            const { Item: admin } = await awsClient_1.docClient.send(new lib_dynamodb_1.GetCommand({
     ...
     */
    static async sendCustomerInquiryConfirmation(userId, inquiry) {
        // 1. In-App Notification
        await (0, user_service_1.saveUserNotification)(userId, {
        ...
        // 2. Push Notification
        try {
            const { Item: user } = await awsClient_1.docClient.send(new lib_dynamodb_1.GetCommand({
                TableName: awsClient_1.MAIN_TABLE,
     ...
     */
    static async sendInquiryStatusUpdate(userId, inquiry, status) {
     ...
     */
    static async sendPush(token, payload, userId) {
        try {
            const message = {
                token: token,
                notification: {
            ...
            const response = await firebaseAdmin_1.firebaseAdmin.messaging().send(message);
