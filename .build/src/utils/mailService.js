 ...
 */
class MailService {
    static getSignatureLogoPath() {
        // Return absolute path to the signature logo relative to this file
        return path_1.default.resolve(__dirname, '..', '..', '..', 'logo_concept_10_signature_thread_1774216216632.png');
    }
    /**
     ...
     */
    static async sendSubscriptionConfirmation(to, name) {
    ...
    static getSubscriptionTemplate(name) {
     ...
     */
    static async sendOrderConfirmedEmail(to, name, order, items, pdfBuffer) {
        const html = this.getOrderConfirmedTemplate(name, order, items);
        const logoPath = this.getSignatureLogoPath();
        const attachments = [{ filename: 'logo.png', path: logoPath, cid: 'brandlogo' }];
        if (pdfBuffer) {
            attachments.push({
     ...
     */
    static async sendOrderStatusEmail(to, name, order, status) {
     ...
     */
    static async sendNewProductEmail(to, name, product) {
     ...
     */
    static async generatePersonnelPDF(employee) {
        ...
        return new Promise((resolve) => {
            ...
            if (employee.maritalStatus === 'Married')
                drawField('Spouse Name', employee.spouseName, 300, y);
            y += 34;
     ...
     */
    static async sendOnboardingWelcomeEmail(to, employee) {
        try {
    ...
    static getOnboardingWelcomeTemplate(name) {
        return `
     ...
     */
    static async sendBirthdayEmail(to, name) {
     ...
     */
    static async sendPayrollPaymentEmail(to, name, data) {
     ...
     */
    static async sendPayrollLedgerEmail(to, name, records) {
     ...
     */
    static async sendCustomBroadcastEmail(to, name, title, body, image, product) {
    ...
    static getPayrollPaymentTemplate(name, data) {
        const accentColor = '#3b82f6';
        return `
    ...
    static getPayrollLedgerTemplate(name, records) {
    ...
    static getOrderConfirmedTemplate(name, order, items) {
     ...
     */
    static async sendBulkInquiryNotification(inquiry) {
        ...
        try {
            await this.transporter.sendMail({
                ...
                attachments: [{
                        filename: 'logo.png',
                        path: path_1.default.join(process.cwd(), '../logo_concept_10_signature_thread_1774216216632.png'),
                        cid: 'brandlogo'
                    }]
            });
     ...
     */
    static async sendInquiryConfirmation(to, name, inquiry) {
     ...
     */
    static async sendStandardInquiryConfirmation(to, name, inquiry) {
     ...
     */
    static async sendStandardInquiryAdminNotification(inquiry) {
     ...
     */
    static async sendInquiryStatusEmail(to, name, inquiry, status) {
    ...
    static getBulkInquiryAdminTemplate(inquiry) {
        return `
      <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; border-radius: 20px; overflow: hidden; background: #fff;">
    ...
    static getInquiryConfirmationTemplate(name, inquiry) {
        return `
    ...
    static getInquiryStatusUpdateTemplate(name, inquiry, status) {
    ...
    static getStandardInquiryConfirmationTemplate(name, inquiry) {
        return `
    ...
    static getStandardInquiryAdminTemplate(inquiry) {
        return `
      <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; border-radius: 20px; overflow: hidden; background: #fff;">
        <div style="background: #000; padding: 30px; text-align: center;">
          <img src="cid:brandlogo" style="width: 120px;">
    ...
    static getCustomBroadcastTemplate(name, title, body, image, product) {
     ...
     */
    static async sendQuotationEmail(to, name, quote, customPdfBuffer) {
        const html = this.getQuotationTemplate(name, quote);
        const logoPath = this.getSignatureLogoPath();
        try {
            const pdfBuffer = customPdfBuffer || await pdfService_1.PDFService.generateQuotation(quote);
            await this.transporter.sendMail({
                from: MailService.DEFAULT_FROM,
                to,
    ...
    static getQuotationTemplate(name, quote) {
...
MailService.transporter = process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD
    ...
    : nodemailer_1.default.createTransport({
        SES: { sesClient: awsClient_1.sesClient, SendEmailCommand: client_sesv2_1.SendEmailCommand },
    });
MailService.DEFAULT_FROM = `"WearDynamite" <${process.env.GMAIL_USER || process.env.SES_FROM_EMAIL || 'noreply@weardynamite.com'}>`;
