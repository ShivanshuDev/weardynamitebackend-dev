...
export const addInventory = async (data: any) => {
  ...
  // ✅ Transform items
  const items = data.items.flatMap((item: any) => {
    return item.variants.map((v: any) => ({
      product_id: item.productId || uuidv4(),
 ...
 * GET INVOICE
 */
export const getInvoice = async (invoice_number: string) => {
 ...
 * PRODUCT HISTORY
 */
export const getProductHistory = async (product_id: string) => {
  const { Items } = await docClient.send(
    new QueryCommand({
      TableName: INVENTORY_TABLE,
      IndexName: 'GSI2',
...
export const createInventoryInvoice = addInventory;